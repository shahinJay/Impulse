#pragma once
#include <math.h>
#include <iostream>

struct vec2 {
    double x;
    double y;
};

inline bool compare_vecs(const vec2& a, const vec2& b) {
    return ((a.x == b.x) && (a.y == b.y));
}

inline vec2 shift(vec2& origin, vec2& point) {
    vec2 res;

    res.x = point.x - origin.x;
    res.y = point.y - origin.y;

    return res;
}

inline vec2 add_vecs(const vec2& a, const vec2& b) {
    return { a.x + b.x, a.y + b.y };
}

inline vec2 scale_vec(const vec2& a, double c) {
    vec2 res{ 0,0 };

    res.x = a.x * c;
    res.y = a.y * c;

    return res;
}

inline double length(const vec2& a) {
    return sqrt((a.x * a.x) + (a.y * a.y));
}

inline double distance(const vec2& a, const vec2 b) {
    double dist = sqrt((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y));
    return dist;
}

inline vec2 normalize(const vec2& a) {
    if (length(a) > 0.001)
        return scale_vec(a, 1 / length(a));
    return { 0,0 };
}

inline vec2 direction(const vec2& from, const vec2& to) {
    vec2 dir = vec2(to.x - from.x, to.y - from.y);
    return normalize(dir);
}

inline void log_vec(std::string& name, const vec2& vec) {
    std::cout << name;
    std::cout << " : ";
    std::cout << vec.x;
    std::cout << " , ";
    std::cout << vec.y;
    std::cout << " |||" << std::endl;
}
